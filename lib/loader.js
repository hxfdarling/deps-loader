...
module.exports = async function(content) {
		...
		data = data
			...
			.map(item => {
				...
				return path.join(
					...
					item
					.replace(/^(B|SFX|pagex|_overridex|SF|page|override)/, $1 => _isFmk ? aliasMapFramework[$1] : aliasMap[$1])
					.replace(/\./g, "/") + '.js')
			})
	...
	async function getRequire(file) {
		...
		deps.forEach(dep => {
			ps.push(fs.exists(dep).then(exists => {
				if (exists) {
					...
					console.log(chalk.yellow(`"${JSON.stringify(dep)}" module no found! \r\n\tplease check ${JSON.stringify(file)} deps file `))
				}
				return ''
			}))
		})
		...
		ps.push(fs.exists(relativeCss).then(exists => {
			if (exists) {
				return `require(${JSON.stringify(relativeCss)})`
			}
			return ''
		}))
		//相对目录下面的html文件
		...
		ps.push(fs.exists(relativeHtml).then(exists => {
			if (exists) {
				...
				return `let __html__ = require(${JSON.stringify(relativeHtml)})
        try{
